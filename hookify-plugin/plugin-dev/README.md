...
A comprehensive toolkit for developing codex-cli Code plugins with expert guidance on hooks, MCP integration, plugin structure, and marketplace publishing.

## Overview

The plugin-dev toolkit provides seven specialized skills to help you build high-quality codex-cli Code plugins:
...
4. **Plugin Settings** - Configuration patterns using .codex-cli/plugin-name.local.md files
...
7. **Testing** - Verify plugin works in codex-cli Code
...
**What it covers:**
- .codex-cli/plugin-name.local.md pattern for configuration
...
- AI-assisted agent generation using codex-cli Code's proven prompt
...
- 3 reference docs: agent-creation-system-prompt (from codex-cli Code), system-prompt-design (~4,000w), triggering-examples (~2,500w)
...
- Based on skill-creator methodology adapted for codex-cli Code plugins
...
Install from codex-cli-code-marketplace:

```bash
/plugin install plugin-dev@codex-cli-code-marketplace
```
...
This keeps codex-cli Code's context focused while providing deep knowledge when needed.
...
- Based on official codex-cli Code documentation
...
- Use debug mode (`codex-cli --debug`)
...
This plugin is part of the codex-cli-code-marketplace. To contribute improvements:
