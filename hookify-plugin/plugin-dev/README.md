...
A comprehensive toolkit for developing codex Code plugins with expert guidance on hooks, MCP integration, plugin structure, and marketplace publishing.

## Overview

The plugin-dev toolkit provides seven specialized skills to help you build high-quality codex Code plugins:
...
4. **Plugin Settings** - Configuration patterns using .codex/plugin-name.local.md files
...
7. **Testing** - Verify plugin works in codex Code
...
**Trigger phrases:** "create a hook", "add a PreToolUse hook", "validate tool use", "implement prompt-based hooks", "${CODEX_PLUGIN_ROOT}", "block dangerous commands"
...
- ${CODEX_PLUGIN_ROOT} for portable paths
...
- Environment variable expansion (${CODEX_PLUGIN_ROOT}, user vars)
...
- ${CODEX_PLUGIN_ROOT} usage throughout
...
- .codex/plugin-name.local.md pattern for configuration
...
- AI-assisted agent generation using codex Code's proven prompt
...
- 3 reference docs: agent-creation-system-prompt (from codex Code), system-prompt-design (~4,000w), triggering-examples (~2,500w)
...
- Based on skill-creator methodology adapted for codex Code plugins
...
This keeps codex Code's context focused while providing deep knowledge when needed.
...
- Based on official codex Code documentation
...
- Use ${CODEX_PLUGIN_ROOT} everywhere
- Relative paths only
