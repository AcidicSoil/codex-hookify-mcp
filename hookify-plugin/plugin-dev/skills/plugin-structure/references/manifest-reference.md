...
The manifest MUST be in the `.codex-plugin/` directory at the plugin root. codex Code will not recognize plugins without this file in the correct location.
...
- Plugin identification in codex Code
...
When codex Code loads components:
...
codex Code validates the manifest on plugin load:
