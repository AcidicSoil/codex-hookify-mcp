...
**Required path**: `.codex-cli-plugin/plugin.json`

The manifest MUST be in the `.codex-cli-plugin/` directory at the plugin root. codex-cli Code will not recognize plugins without this file in the correct location.
...
- Plugin identification in codex-cli Code
...
When codex-cli Code loads components:
...
codex-cli Code validates the manifest on plugin load:
